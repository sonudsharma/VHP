...
export class UserProfileComponent implements OnInit {
  ...
  onSubmit(post, from, align): void {
    this.id1 = this.id.toString();
    this.userService.uploadDocument(this.myFile, this.id1).subscribe(

      data => {
        const success = data.success;
        const type1 = success == '1' ? ['success'] : ['danger'];
